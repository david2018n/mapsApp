...
export class MapService {
  ...
  flyto(coords: LngLatLike) {
    ...
    if (!this.isMapReady) throw Error('Mapa no iniciado');
    this.map?.flyTo({
      zoom: 14,
      center: coords
    })
  ...
  private drawPolyline(route: Route) {
    ...
    if( this.map.getLayer('RoteString') ){
      this.map.removeLayer('RoteString');
      this.map.removeSource('RoteString');
    }

    this.map.addSource('RoteString', sourceData);
